...
public class TaskController {
	...
	public ResponseEntity newMultipleChoice(@Valid @RequestBody NewTaskWithOptionsDTO newTaskWithOptionsDTO) {
		newTaskWithOptionsDTO.setType(Type.MULTIPLE_CHOICE);

		if (newTaskWithOptionsDTO.getOptions().isEmpty()) {
			return ResponseEntity.status(HttpStatus.BAD_REQUEST)
					.body(new ErrorItemDTO("option", "A atividade deve ter no minimo 3 e no máximo 5 alternativas"));
		}

		try {
			taskService.checkNumberOfOptionsMultipleChoice(newTaskWithOptionsDTO);
			taskService.checkNumberOfCorrectAndIncorrectAnswersMultipleChoice(newTaskWithOptionsDTO);
			taskService.checkTaskStatementDiffersFromOption(newTaskWithOptionsDTO);
